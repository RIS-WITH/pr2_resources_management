...
public:
    ...
    RosNodeFixture(){
        ...
        for(auto &n : reactive_input_names){
            reactive_input_publishers.emplace_back(new MessageGenerator<::pr2_head_manager_msgs::Point>(nh,"/pr2_head_manager",n));
            reactive_input_publishers.emplace_back(new MessageGenerator<::pr2_head_manager_msgs::PitchYaw>(nh,"/pr2_head_manager",n));
            reactive_input_publishers.emplace_back(new MessageGenerator<::pr2_head_manager_msgs::PrioritizedPitch>(nh,"/pr2_head_manager",n));
            reactive_input_publishers.emplace_back(new MessageGenerator<::pr2_head_manager_msgs::PrioritizedYaw>(nh,"/pr2_head_manager",n));
        }
        set_priorities = nh.advertise<resource_management_msgs::PrioritiesSetter>("/pr2_head_manager/set_priorities",100,/*latch=*/true);
        ...
        active_buffer = nh.subscribe<std_msgs::String>("/pr2_head_manager/active_buffer",10,cb);
...
public:
    ...
    {
        state_machine_client = nh.serviceClient<pr2_head_manager_msgs::StateMachineRegister>("/pr2_head_manager/state_machines_register");
    }

    pr2_head_manager_msgs::StateMachineRegister makeStateMachine(std::string initial, double timeout, double dl_in_secs_from_now, int prio){
        pr2_head_manager_msgs::StateMachineRegister sig;
    ...
    void addState(pr2_head_manager_msgs::StateMachineRegister::Request &request, const std::string &id, const std::vector<::resource_management_msgs::StateMachineTransition> & transitions = {}){
        pr2_head_manager_msgs::StateMachineStatePoint state;
        state.header.id=id;
        state.header.transitions = transitions;
        request.state_machine.states_Point.push_back(state);
    }
    ...
    pr2_head_manager_msgs::StateMachineRegister makeSimpleStateMachine(int prio){
...
int main(int argc, char *argv[]){
    ...
    ros::init(argc,argv,"tests_pr2_head_manager");
    ros::NodeHandle nh;
